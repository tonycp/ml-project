...
\begin{abstract}
    Este documento detalla el desarrollo de un sistema de predicción de tráfico aéreo para la FIR Habana mediante modelos de aprendizaje automático. Se aborda la problemática crítica de gestionar eficientemente el flujo de aeronaves en un corredor estratégico del Caribe. La investigación implementa un pipeline completo de ingeniería de datos que integra registros ATFM, variables meteorológicas y eventos noticiosos. Un hallazgo metodológico clave fue la corrección del conteo de volumen mediante Identificadores Únicos Globales de Vuelo (GUFI), eliminando el sesgo de doble contabilización por sector. Se evaluaron múltiples arquitecturas (ARIMA, Prophet, Random Forest, XGBoost, LSTM), determinándose que XGBoost alcanza el mejor rendimiento absoluto con un MAE de 121.85 vuelos, mientras que Random Forest ofrece la mayor estabilidad operativa (σ=9.86). La mejor configuración promedio integra variables de aerolíneas, clima y noticias, logrando una mejora del 27.61\% respecto al baseline. Los resultados validan la viabilidad de desplegar estos modelos como herramientas de soporte a la planificación operativa de turnos de control.

    \textbf{Palabras clave:} Gestión de Tráfico Aéreo, ATFM, Aprendizaje Automático, XGBoost, Random Forest, Series Temporales, GUFI.
\end{abstract}

\renewcommand{\abstractname}{Abstract}
\begin{abstract}

    This document details the development of an air traffic forecasting system for Havana FIR using machine learning models. It addresses the critical challenge of efficiently managing aircraft flow in a strategic Caribbean corridor. The research implements a complete data engineering pipeline integrating ATFM records, meteorological variables, and news events. A key methodological finding was the correction of volume counting using Global Unique Flight Identifiers (GUFI), eliminating double-counting bias per sector. Multiple architectures were evaluated (ARIMA, Prophet, Random Forest, XGBoost, LSTM), determining that XGBoost achieves the best absolute performance with a MAE of 121.85 flights, while Random Forest offers the highest operational stability (σ=9.86). The best average configuration integrates airline, weather, and news variables, achieving a 27.61\% improvement over baseline. Results validate the feasibility of deploying these models as operational planning support tools for air traffic control shift scheduling.

    \textbf{Keywords:} Air Traffic Management, ATFM, Machine Learning, XGBoost, Random Forest, Time Series, GUFI.
\end{abstract}
...
% --- COMIENZO DEL TEXTO NUEVO ---
\subsection{Investigación Previa y Motivación para un Estudio Comparativo}
El desarrollo del presente sistema predictivo tuvo como punto de partida una investigación exploratoria centrada en la aplicabilidad de \textit{Facebook Prophet} al contexto de la FIR Habana \cite{largacha2025prophet}. Dicho estudio, realizado sobre datos históricos del mismo corredor aéreo, confirmó la capacidad de Prophet para capturar eficientemente las estacionalidades múltiples (horaria, diaria, semanal) inherentes al tráfico aéreo, logrando una reducción del Error Absoluto Medio (MAE) del 22\% respecto a los modelos lineales utilizados tradicionalmente en la planificación operativa local.

Sin embargo, dicho trabajo identificó una limitación crítica para el despliegue operativo: el desempeño de Prophet se degrada significativamente en la predicción de \textbf{picos operacionales} (MAPE del 94.6\% en el percentil 95 de densidad). Este comportamiento se atribuye al tratamiento de valores extremos como ruido ($\epsilon_t$) dentro de su modelo aditivo generalizado, en lugar de como componentes estructurales del sistema. Además, el modelo mostró una alta sensibilidad a la acumulación de incertidumbre en horizontes superiores a 24 horas, con una cobertura de intervalos de confianza que caía por debajo del 80\%.

Estos hallazgos establecieron un \textbf{benchmark inicial} y plantearon preguntas clave: ¿Es esta limitación una propiedad inherente de Prophet, o existen otros algoritmos más adecuados para manejar la variabilidad extrema y la no-linealidad del tráfico aéreo en la región? Para responder a esto, y con el objetivo de desarrollar una herramienta con la estabilidad necesaria para un entorno de seguridad crítica, la presente investigación expande el enfoque hacia una \textbf{evaluación comparativa sistemática} de múltiples familias de algoritmos. Se busca así identificar no solo el modelo con mejor precisión puntual, sino aquel que ofrezca el mejor equilibrio entre rendimiento absoluto, estabilidad operativa y capacidad para generalizar en regímenes de alta densidad.
% --- FIN DEL TEXTO NUEVO ---
...
Los modelos ARIMA (AutoRegressive Integrated Moving Average) y su variante estacional SARIMA constituyen la línea base clásica para la predicción de series temporales. Estos modelos asumen que los valores futuros son una función lineal de los valores pasados y los errores pasados \cite{hyndman2018}. Por otro lado, \textbf{Prophet} es un modelo aditivo desarrollado por Facebook, diseñado para manejar series temporales con fuertes efectos estacionales y días festivos. Descompone la serie en componentes de tendencia, estacionalidad y eventos, siendo particularmente robusto frente a valores atípicos y cambios en la tendencia \cite{taylor2018}.

Un estudio precursor aplicado a la FIR Habana \cite{largacha2025prophet} demostró la eficacia de Prophet para capturar estacionalidades múltiples (diarias, semanales), logrando una reducción del 22\% en el MAE respecto a modelos lineales tradicionales. Sin embargo, dicho estudio también reveló una limitación crítica: Prophet tiende a tratar los picos operacionales extremos como ruido, subestimándolos sistemáticamente (MAPE del 94.6\% en el percentil 95 de densidad). Esta característica, junto con su degradación en horizontes superiores a 24 horas, motivó su inclusión en el presente estudio como punto de referencia clave. El objetivo es contrastar su desempeño, especialmente en condiciones de alta congestión, con modelos capaces de capturar relaciones no lineales más complejas y ofrecer mayor estabilidad predictiva en escenarios operativos críticos.
...
\subsubsection{Estrategias de Codificación de Variables Categóricas}
Dado que tanto los datos de aerolíneas como las noticias son de naturaleza categórica, se evaluaron dos enfoques de preprocesamiento para su integración en los modelos:

\textbf{Codificación One-Hot (OneHot=True):}
Esta estrategia crea una variable binaria independiente para cada categoría única. Para el caso de aerolíneas, cada operador (e.g., American Airlines, Copa Airlines) recibe su propia columna con valores 0/1 indicando presencia/ausencia en el día. Para noticias, cada categoría temática (política, deportes, economía) se representa de forma similar.

\textit{Ventajas:} Permite al modelo aprender patrones específicos por aerolínea o tipo de evento, capturando efectos individuales como la estacionalidad particular de rutas turísticas vs comerciales.

\textit{Desventajas:} Incrementa significativamente la dimensionalidad del espacio de características (+50 columnas para aerolíneas), lo que puede causar dispersión de datos en modelos lineales.

\textbf{Codificación Agregada (Aggregated):}
En este enfoque, se calculan estadísticas agregadas por categoría. Para aerolíneas, se computa el conteo total de vuelos por operador en la ventana temporal. Para noticias, se agrega el número de eventos por categoría temática.

\textit{Ventajas:} Reduce drásticamente la dimensionalidad, generando solo una variable numérica por fuente de datos.

\textit{Desventajas:} Pierde la granularidad individual, imposibilitando la detección de patrones específicos de aerolíneas o eventos particulares.

El análisis experimental reveló que para datos de aerolíneas, la codificación One-Hot fue superior (mejora promedio +26.70\% vs baseline), mientras que para noticias, la estrategia agregada mostró resultados ligeramente mejores en configuraciones complejas, sugiriendo que el impacto noticioso es más difuso y menos específico por categoría.
...
En esta sección se presentan los resultados experimentales obtenidos evaluando el desempeño de los modelos sobre datos agregados diariamente. Se prioriza este horizonte temporal por su relevancia para la planificación estratégica de recursos a mediano plazo.
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{lccc}
            \toprule
            \textbf{Configuración} & \textbf{Mejor Modelo} & \textbf{MAE} & \textbf{Mejora vs Baseline} \\
            \midrule
            BASELINE & Random Forest & 155.17 & -- \\
            WEATHER & Random Forest & 140.94 & +14.84\% \\
            AEROLINEAS (OneHot) & Random Forest & 124.15 & +26.70\% \\
            AEROLINEAS + WEATHER & Random Forest & 123.79 & +27.32\% \\
            \textbf{AEROLINEAS + NEWS + WEATHER} & \textbf{Random Forest} & \textbf{124.19} & \textbf{+27.61\%} \\
            AEROLINEAS + NEWS (Agg) & \textbf{XGBoost} & \textbf{121.85} & +26.09\% \\
            \bottomrule
        \end{tabular}
    \end{table}

    El mejor rendimiento absoluto fue alcanzado por \textbf{XGBoost} con la configuración ``Aerolíneas + Noticias Agregadas`` (MAE: 121.85 vuelos). Sin embargo, la configuración que ofrece la mejor mejora promedio a través de todos los modelos evaluados es ``Aerolíneas + Noticias + Clima`` (+27.61\%), donde Random Forest alcanza un MAE de 124.19 vuelos.
    ...
    \begin{itemize}
        \item \textbf{Random Forest ($\sigma = 9.86$):} Mostró la mayor estabilidad entre los modelos de aprendizaje automático, produciendo resultados consistentes independientemente de pequeñas variaciones en los datos de entrenamiento.
        \item \textbf{XGBoost ($\sigma = 20.27$):} Aunque capaz de alcanzar el mejor MAE absoluto, mostró más del doble de variabilidad que Random Forest.
        \item \textbf{LSTM ($\sigma = 143.48$):} Exhibió una inestabilidad significativa en este dataset diario, descartándose para uso operativo.
        \item \textbf{ARIMA/Prophet ($\sigma = 0$):} Modelos determinísticos sin variabilidad, pero con MAE muy superiores (>330).
    \end{itemize}
...
\section{Discusión}
    ...
    Un hallazgo crítico para la implementación es el compromiso entre potencia y estabilidad. XGBoost alcanzó el mejor MAE absoluto (121.85), pero exhibió una desviación estándar ($\sigma = 20.27$) más del doble que Random Forest ($\sigma = 9.86$). En un entorno de seguridad crítica como el control de tráfico aéreo, la consistencia predictiva de Random Forest lo convierte en la opción más segura para despliegue operativo, a pesar de tener un MAE ligeramente superior (123.79-124.19 según configuración).
...
\section{Conclusiones}
    ...
    \begin{itemize}
        \item \textbf{Supremacía de la Programación Planificada:} La variable más determinante para reducir el error de predicción fue la inclusión explícita de los operadores aéreos. La mejor configuración promedio (Aerolíneas + Noticias + Clima) logró una mejora del 27.61\% respecto al baseline, confirmando que el tráfico aéreo es un sistema altamente estructurado donde la ``intención`` de vuelo pesa más que las variables ambientales locales.
        ...
        \item \textbf{Trade-off Precisión vs Estabilidad:} XGBoost alcanzó el mejor MAE absoluto (121.85), pero Random Forest demostró ser más estable ($\sigma = 9.86$ vs $\sigma = 20.27$). Para despliegue operativo en entornos críticos, la consistencia de Random Forest (MAE 123.79-124.19) lo convierte en la opción más segura.
    ...
    \begin{enumerate}
        ...
        \item \textbf{Sistema de Alertas de Desviación:} Implementar un monitor que compare el tráfico real vs predicho en ventanas de 4 horas; una divergencia sostenida $>20\%$ debería disparar alertas de ``Evento Anómal`` no capturado por el modelo base.
